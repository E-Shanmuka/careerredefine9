...
export const createQuery = async (req, res) => {
  try {
    ...
    const userEmail = userId ? newQuery.user.email : email;
    const userName = userId ? newQuery.user.name : name;
    const userEmailInstance = new Email(
      { email: userEmail, name: userName },
      `${req.protocol}://${req.get('host')}/queries/${newQuery._id}`
    );
    ...
    if (admin) {
      const adminEmail = new Email(
        ...
        `${req.protocol}://${req.get('host')}/admin/queries/${newQuery._id}`
      );
      await adminEmail.send('adminNewQuery', 'New Query Received', {
        adminName: admin.name,
        userName: userId ? newQuery.user.name : name,
        userEmail: userId ? newQuery.user.email : email,
        subject: newQuery.subject,
        message: newQuery.message,
        adminUrl: `${req.protocol}://${req.get('host')}/admin/queries/${newQuery._id}`,
      });
    }
